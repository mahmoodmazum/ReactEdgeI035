...
function userReducer(state, action) {
  switch (action.type) {
    case "OPEN_EDIT":
      return { ...state, form: { ...state.user }, isEditing: true, saved: false };
    case "CLOSE_EDIT":
    ...
    case "SAVE_SUCCESS":
      return { ...state, user: action.payload, loading: false, isEditing: false, saved: true };
    default:
      return state;
  }
}
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-6 max-w-sm mx-auto">
      ...
      {!isEditing ? (
        <div className="space-y-2">
          ...
          {saved && (
            <p className="text-green-600 text-xs mt-2 font-medium">Saved via mock API!</p>
          )}
        </div>
      ) : (
        /* Edit box */
